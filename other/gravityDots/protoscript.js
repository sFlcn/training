import { generateDotsCluster, getDistance, getAngle } from './dots.js';
...
function drawDots() {
  ctx.fillStyle = 'rgba(5, 5, 5, 0.33)';
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  for (let i = 0; i < dotsArray.length; i++) {
    for (let j = i+1; j < dotsArray.length; j++) {
      const d = getDistance(dotsArray[i], dotsArray[j]);
      if (d > 100) {
        const f = gravityElement.value /d;
        dotsArray[i].recalc(f, getAngle(dotsArray[i], dotsArray[j]));
        dotsArray[j].recalc(f, getAngle(dotsArray[j], dotsArray[i]));
      }
    }
  }
