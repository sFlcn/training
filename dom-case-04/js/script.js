...
const siteGlossary = document.querySelector('.glossary');
const modalWindow = document.querySelector('.modal');
const modalWindowCloseButton = document.querySelector('.modal .close-button');
const modalContent = document.querySelector('.modal-content');

function createNavItemString({key, items}) {
  return `
...
function createGlossaryItemsString({key, items}) {
  return `
...
function createModalContent({description, cite}) {
  return `
  ${description}
  <cite>${cite}</cite>
  `;
...
function onNavItemClick(evt) {
  ...
  if (sectionId && sectionId !== '#') {
    const glossarySection = document.querySelector(sectionId);
    if (glossarySection) {
...
function showPopup(popupElement, popupCloseButton) {

  function closePopup() {
    popupCloseButton.removeEventListener('click', closeButtonHandler);
    document.removeEventListener('keydown', escKeydownHandler);
    popupElement.classList.remove('shown');
  }
...
function renderPopup(clickedTerm) {
  const termsGroup = terms.find(({key}) => {
    return clickedTerm.toUpperCase().indexOf(key.toUpperCase()) === 0;
  });

  if (termsGroup && termsGroup.items) {
    const currentTerm = termsGroup.items.find((item) => {
      return item.term === clickedTerm;
    });

    if (currentTerm) {
      modalContent.innerHTML = '';
      modalContent.insertAdjacentHTML('beforeend', createModalContent(currentTerm));
    }
  }
}

function onTermClick(evt) {
  const termLink = evt.target.closest('a');
  if (!termLink) {
    return;
  }

  evt.preventDefault();
  const {term} = termLink.dataset;

  renderPopup(term);
  showPopup(modalWindow, modalWindowCloseButton);
}

function render() {
  const navElementsString = terms.map(createNavItemString).join('');
  const glossaryElementsString = terms.map(createGlossaryItemsString).join('');

  siteNav.innerHTML = navElementsString;
  siteGlossary.innerHTML = glossaryElementsString;
}

function init() {
  render();
  siteNav.addEventListener('click', onNavItemClick);
  siteGlossary.addEventListener('click', onTermClick);
}
